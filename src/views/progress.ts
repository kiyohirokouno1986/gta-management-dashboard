...
export function progressPanel(ctx: Ctx): string {
  ...
  // 財務実績（MoneyForward）の営業利益。最新実績月まで。
  const opF = ctx.fin.op.slice(0, i + 1);
  const hasFin = opF.length > 0;
  ...
  const oc = hasFin
    ? `累計 実績(管理) ${mm(opAC[i])}万 ／ 財務(MF) ${mm(opFC[i])}万 ／ 計画 ${mm(opPC[i])}万`
    : `累計 実績 ${mm(opAC[i])}万 ／ 計画 ${mm(opPC[i])}万`;
  const finLegend = hasFin
    ? `　<span style="color:#D85A30;font-weight:700">┄</span> 財務実績(MF)`
    : "";
  return `<div style="font-size:18px;font-weight:700;margin-bottom:2px">進捗（計画 対 実績）</div>
  <div style="font-size:12px;color:#888780;margin-bottom:12px">全社・単位 万円　<span style="color:#CECBF6;font-weight:700">━</span> 計画　<span style="color:#534AB7;font-weight:700">━</span> 実績(管理)${finLegend}</div>
  ${sec("売上", uc, svgChart(uriP, uriA, false), svgChart(uriPC, uriAC, false))}
  ${sec("営業利益", oc, svgChart(opP, opA, true, hasFin ? opF : undefined), svgChart(opPC, opAC, true, hasFin ? opFC : undefined))}
  <div style="font-size:11px;color:#888780">0ライン超えが黒字。<b>実績(管理)</b>＝部門別PLの部門利益合計、<b>財務(MF)</b>＝MoneyForwardの営業利益（会計基準差で管理会計と一致しない）。営業利益は計画で5月以降プラスに転じ、通期で +2,579万（黒字化）を目指す。</div>`;
}
